...
var venueObjects = [];
    ...
    var totalDuration = _.sum(_.values('time.duration')); //Attempt 1: sum all the time.duration values for a venueName
    var venueObj = { venue:venueName, duration:totalDuration[venueName], //Attempt 1: return the sum for each venue and store in array
    numShows:venueShows[venueName].length };
    venueObjects.push(venueObj)
    };
    console.log(venueObj);

var durationByVenue = []; //Atempt 2: Create an array that goes through each venue and sums all the values for duration for each venue
    for (i=0; i<venues.length; i++){
    var totalDuration = _.sum(_.values(venues, 'duration')); 
    console.log(durationByVenue);
    };
